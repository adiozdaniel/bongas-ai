//! Resilience configuration with Builder pattern and validation.
//!
...
/// Validation error for resilience configuration.
...
impl std::fmt::Display for ResilienceConfigError {
...
impl std::error::Error for ResilienceConfigError {}

// ─── Configuration ──────────────────────────────────────────────────────────

/// Configuration for the resilience system.
///
/// All fields are private for encapsulation. Use `ResilienceConfig::builder()`
...
/// Builder for `ResilienceConfig` with validation.
#[derive(Debug, Clone)]
...
impl ResilienceConfigBuilder {
    ...
    pub fn validate(&self) -> Result<(), ResilienceConfigError> {
        ...
        if self.rate_interval.is_zero() {
            return Err(ResilienceConfigError::InvalidRateInterval);
        }

        if self.histogram_precision < 1 || self.histogram_precision > 5 {
            return Err(ResilienceConfigError::InvalidHistogramPrecision);
        }

        if let Some(export_interval) = self.export_interval {
            if export_interval.is_zero() {
                return Err(ResilienceConfigError::InvalidExportInterval);
            }
        }

        if let Some(retention) = self.retention_period {
            if retention.is_zero() {
                return Err(ResilienceConfigError::InvalidRetentionPeriod);
            }
        }
        ...
        if let Some(export_interval) = self.export_interval {
            if export_interval < self.rate_interval {
                return Err(ResilienceConfigError::ExportIntervalTooShort);
            }
        }
    ...
    pub fn build(self) -> Result<ResilienceConfig, ResilienceConfigError> {
        self.validate()?;
