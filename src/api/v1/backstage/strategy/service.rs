...
pub async fn create_scenario(
    ...
    let config = engine.governance.scenario_factory.repo().create(payload).await?;
    
    info!(request_id = %request_id, scenario = %config.scenario.slug, "New scenario configuration created");
    Ok(Json(StandardResponse::success(config).with_request_id(request_id)))
}

/// GET /api/v1/recommendation/admin/scenarios/active
pub async fn list_active_scenarios(
    headers: HeaderMap,
    Extension(engine): Extension<Arc<BongasEngine>>,
) -> Result<Json<StandardResponse<Vec<ScenarioWithStrategy>>>, AppError> {
    let request_id = extract_request_id_from_headers(&headers);
    let configs = engine.governance.scenario_factory.repo().find_all_active().await?;
    Ok(Json(StandardResponse::success(configs).with_request_id(request_id)))
}

/// GET /api/v1/recommendation/admin/scenarios/:slug
pub async fn get_scenario_config(
    headers: HeaderMap,
    ...
    let config: ScenarioWithStrategy = engine.governance.scenario_factory.repo().find_by_slug(&slug).await?
        .ok_or_else(|| AppError::NotFound(format!("Scenario {} not found", slug)))?;
        
    Ok(Json(StandardResponse::success(config).with_request_id(request_id)))
}

/// PATCH /api/v1/recommendation/admin/scenarios/:slug
pub async fn update_scenario(
    ...
    let request_id = extract_request_id_from_headers(&headers);
    let config = engine.governance.scenario_factory.repo().update(&slug, payload).await?;
    
    info!(request_id = %request_id, scenario = %slug, "Scenario configuration updated");
    
    // Auto-reload the updated scenario in the engine
    let _ = engine.reload_scenario(&slug).await;
...
pub async fn delete_scenario(
    ...
    let request_id = extract_request_id_from_headers(&headers);
    engine.governance.scenario_factory.repo().delete(&slug).await?;
    
    // Remove from engine memory
    engine.remove_scenario(&slug).await;
    
    info!(request_id = %request_id, scenario = %slug, "Scenario configuration deleted");
    Ok(Json(StandardResponse::success(true).with_request_id(request_id)))
}

/// POST /api/v1/recommendation/admin/reload
