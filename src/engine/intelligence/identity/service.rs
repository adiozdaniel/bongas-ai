...
impl IdentityStitcher {
    ...
    pub async fn reactive_stitch(
        ...
        if let Ok(Some(_)) = engine.cache.get::<String>(&cache_key).await {
            return Ok(());
        }
        ...
        tokio::spawn(async move {
            ...
            let _ = engine_clone.cache.set_with_ttl(&cache_key, &"1".to_string(), std::time::Duration::from_secs(86400)).await;
